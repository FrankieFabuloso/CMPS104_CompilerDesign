...
// re-working of the given ast-tree struct way in a more readable way.
...
// adopt three ast's like adopt2 above
...
// a combonation of adopt1 and change symbol
...
// Re-adopt children from the passed node laterally.
astree* adopt_all(astree *root, astree* child) {
   printf("%s will kidnap: \n", get_yytname(root->symbol));
   dump_astree(stdout, child);
   // pop all the children and readopt them for given root
...
static void dump_node (FILE* outfile, astree* node) {
   ...
   fprintf(outfile, "%s \"%s\" %zu.%zu.%zu\n", tname,
      (node->clexinfo)->c_str(), node->filenr,
...
// free three ast's like free_ast2
